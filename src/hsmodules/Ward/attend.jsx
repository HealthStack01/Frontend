...
const Attend = ({ appointment, backClick }) => {
	...
	const loadDocuments = documentName => {
	...
	const loadOrders = (orderType, setList) => {
		find(queryOrders(orderType, appointment.clientId))
			.then(response => {
				setList(response.data.filter(obj => obj.documentname !== 'Lab Orders'));
			})
	...
	const changeFormSchema = documentName => {
				...
				{currentDocumentName && (
					<FullDetailsWrapper className='attend attend-medium'>
						<FormRowsView
							...
							schema={
								documentSchemas[currentDocumentName] || GenericTableSchema
							}

     ...
     const handlePay = async(client,i )=>{
       ...
       // console.log(e.target.checked)
        ...
        // console.log(bill)
            ...
            setState((prevstate)=>({...prevstate, financeModule:newProductEntryModule}))
    ...
    return(     
            <>  
                 <div className="level">
                    ...
                    <div className="level-item"> <span className="is-size-6 has-text-weight-medium">Unpaid Invoices/Bills </span></div> 
                ...
                <div className=" pullup shift-right ">
                    <div className ="columns">
                            ...
                            <table className="table is-striped  is-hoverable is-fullwidth is-scrollable mr-2">
                                <thead>
                                    <tr>
                                        ...
                                        <th><abbr title="Amount">Grand Total</abbr></th>
                                        <th><abbr title="Category Amount">Categories Total</abbr></th>
                                        <th><abbr title="Action">Action</abbr></th>
                                        </tr>
                                </thead>
                                <tbody>
                                {facilities.map((Clinic, i)=>( //key={order._id}  /*  onClick={()=>handleMedicationRow(order)} */  }
                                        <tr className={Clinic.client_id===(selectedClient?.client_id||null)?"is-selected":""} key={Clinic.client_id} onClick ={()=>{handleRow(Clinic,i)}}>
                                           ...
                                           {Clinic.clientname}
                                            ...
                                            <td>
                                            {Clinic.clientAmount.toFixed(2)}
                                            </td> 
                                            <td>
                                            
                                            {Clinic.bills.map((category, i)=>(
                                                <p>{category.catName} {category.catAmount.toFixed(2)}</p>
                                            ))}
                                            </td>
                                            <td>
                                                <button className="button is-info is-small" onClick ={()=>{handlePay(Clinic,i)}}>
                                                    PAY
                                                </button>
                                            </td> 
                                            </tr>
                       ...
                       {selectedClient && <div className="column ">
                            <table className="table is-striped  is-hoverable is-fullwidth is-scrollable mr-0 ">
                              ...
                              {/*  <div> {category.catName} with {category.order.length} Unpaid bills.</div> */}
                                            ...
                                            { category.order.map((order, i)=>(
                                                ...
                                                <tr key={i}  /*  onClick={()=>handleMedicationRow(order)} */  className={order._id===(selectedFinance?._id||null)?"is-selected":""}>                                         
                                                <th><input type = "checkbox" name={order._id} onChange={(e)=>handleChoseClient(selectedClient, e,order,)}  checked={order.checked}/>  {i+1}</th>

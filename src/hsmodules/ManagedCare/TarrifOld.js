...
export async function PreAuthorizationList() {
  // const { register, handleSubmit, watch, errors } = useForm();
  // eslint-disable-next-line
  const [error, setError] = useState(false);
  // eslint-disable-next-line
  ...
  const handleSearch = async val => {
    ...
    await ServiceServ.find({ query: query })
      .then(res => {
        console.log(res);
        setFacilities(res.data);
        setMessage(' Tariff  fetched successfully');
        setSuccess(true);
      })
      .catch(err => {
        console.log(err);
        setMessage('Error fetching Tariff, probable network issues ' + err);
        setError(true);
      });
  };
  ...
  useEffect(async () => {
    const fetchTariff = async () => {
      const findTariff = await ServicesServ.find({
        query: {
  ...
  useEffect(() => {
    ...
    ServicesServ.on('created', obj => null);
    ServicesServ.on('updated', obj => null);
    ServicesServ.on('patched', obj => null);
    ServicesServ.on('removed', obj => null);

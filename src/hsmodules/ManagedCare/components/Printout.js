...
export const ProviderPrintout = ({data, action}) => {
  ...
  return (
    <Box sx={{width: "100%", height: "100%"}}>
      ...
      >
        <SendViaEmail closeModal={() => setEmailModal(false)} />
      </ModalBox>
      ...
      >
        ...
        <GlobalCustomButton onClick={() => setEmailModal(true)}>
          Send Via Email
        </GlobalCustomButton>
      </Box>
    </Box>
...
//CONVERT THIS EMAIL FORMAT TO BE PURE HTML WITH INLINED CSS, ALSO USE HTML TABLE TO REPLACE CUSTOMTABLE
const EmailFormat = () => {
  const beneschema = [
    {
      name: "S/N",
...
export const SendViaEmail = ({closeModal}) => {
  ...
  const handleSendEmail = async data => {
    const html = renderToStaticMarkup(<EmailFormat />);

    // const iframeDocument =
    //   invoice.contentDocument || invoice.contentWindow.document;
    //const doc = new XMLSerializer().serializeToString(iframeDocument);
